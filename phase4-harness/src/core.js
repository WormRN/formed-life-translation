...
export async function requestModel(worker,bodyText,signal){
  ...
  else if(worker.provider==='anthropic'){url='https://api.anthropic.com/v1/messages';headers['x-api-key']=key;headers['anthropic-version']='2023-06-01';body={model:worker.model,max_tokens:8000,messages:[{role:'user',content:bodyText}]};parse=j=>({text:(j.content||[]).map(x=>x.text||'').join(''),id:j.id,usage:j.usage||{}})}
