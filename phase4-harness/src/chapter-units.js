...
import {assertPassageSenseResolution,assertLockedMatrixAlerts,assertPacketMatrixHygiene,normalizeMatrixAlertsForPacket,unitSenseMaterial,assertCandidateSenseApparatus,assertCommonBasePromptHash,parseModelJson} from './sense-resolution-core.js';
...
const authorityTypesMarkdown=await fs.readFile(path.resolve(root,'../docs/current/FLT_contextual_lexical_matrix_authority_types_v2.0.md'),'utf8');
const governingMatrixMarkdown=`${matrixMarkdown}\n\n${authorityTypesMarkdown}`;
const sourceLemmaIndex=await json(path.resolve(root,'../data/source/sblgnt_parsed.json'));
assertLockedMatrixAlerts(chapter,{matrixMarkdown:governingMatrixMarkdown,sourceLemmaIndex});
...
const vocabularyBoundary='Unpack a term when its label blocks comprehension—when a religion-naive reader would supply no meaning or a wrong one. Do not unpack a term merely because its sense varies by context; doing so can fix one construal and forfeit live alternatives. Render context-sensitive terms plainly and let context carry the force. When a label is opaque and its sense also varies, treat it as GOVERNED and escalate rather than improvising a uniform unpacking.';
