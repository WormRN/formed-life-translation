...
for(const unit of chapter.units){
  const common={project:{translation_identity:chapter.translation_identity,target_reader:chapter.target_reader,tone:chapter.tone},governing_rules:chapter.governing_rules,existing_decisions:chapter.existing_decisions,unit:{unit_id:unit.unit_id,passage:unit.passage,title:unit.title,known_issues:unit.known_issues,verses:unit.verses},visibility:{sealed_human_unit_absent:!chapter.sealed_unit,comparison_translations_absent:true,benchmark_absent:true}};
  const results=await Promise.all(config.workers.map(w=>call(w,unit,`You are drafting one blind unit for FLT ${chapter.book} ${chapter.chapter}. Mission: ${commonMission} The reading text is firmly dynamic-equivalence; closeness to Greek form is not a virtue by itself. Preserve meaning and avoid unsupported commentary. Do not reconstruct or refer to any sealed human wording. Schema: {"verse_renderings":[{"reference":"${unit.verses[0].reference}","text":"..."}],"paragraph_rendering":"...","dynamic_moves":[{"reference":"...","move":"...","warrant":"..."}],"risks":[{"reference":"...","risk":"..."}],"human_questions":[{"reference":"...","question":"...","options":["...","..."]}]}. MATERIAL:${JSON.stringify(common)}`)));
