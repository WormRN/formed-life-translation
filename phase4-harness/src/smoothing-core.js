...
export function assertSmoothingIntegrity(candidate,output){
  ...
  for(const change of output.changes){
    ...
    if(change.current_wording!==original.get(change.reference))throw new Error(`Current wording mismatch for ${change.reference}; copy the complete exact current verse: ${JSON.stringify(original.get(change.reference))}`);
    if(change.proposed_wording!==proposed.get(change.reference))throw new Error(`Proposed wording mismatch for ${change.reference}; copy the complete exact proposed verse from proposed_verse_renderings.`);
