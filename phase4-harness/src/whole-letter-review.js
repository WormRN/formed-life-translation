...
const crossFinding={...finding,required:['category','severity','assessment','source_or_context_evidence','minimum_action'],properties:{...finding.properties,range:{type:'string',minLength:1}},anyOf:[{required:['reference']},{required:['range']}]};
const sourceSchema={
  type:'object',additionalProperties:false,
  ...
  properties:{
    ...
    cross_letter_findings:{type:'array',maxItems:12,items:crossFinding},
...
const reviews=await Promise.all(config.workers.map(w=>call(w,'whole_letter_source_review',sourcePrompt,validateSource,o=>{for(const f of o.cross_letter_findings){if(!f.reference&&f.range){f.reference=f.range;delete f.range;}}const checks=o.constitutional_checks.map(x=>x.check).sort();if(JSON.stringify(checks)!==JSON.stringify([...requiredChecks].sort()))throw new Error('Every whole-letter constitutional check must appear exactly once.');if(o.chapters_1_2_certification.status==='compatible'&&o.chapters_1_2_certification.findings.some(x=>x.severity==='block'))throw new Error('Compatible certification cannot contain a block.');})));
